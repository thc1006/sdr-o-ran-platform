# SDR-O-RAN Platform - Main Terraform Configuration
...
terraform {
  ...
  backend "s3" {
    ...
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "sdr-oran-terraform-locks"
  }
}
...
# Storage Class for Redis SDL (gp3 with high IOPS)
resource "kubernetes_storage_class" "redis_sdl" {
  metadata {
    name = "redis-sdl-storage"
    annotations = {
