...
RSRP Violation Rate (\%) & 3.48 & 23.25 \\
Mean Tx Power (dBm) & 46.89 & 40.84 \\
\bottomrule
\end{tabular}
\end{table}

The DQN agent learns to increase transmit power by $\sim$6~dB relative to the baseline, which dramatically reduces RSRP violations from 23.25\% to 3.48\% --- a \textbf{6.7$\times$ reduction}. This represents a clear signal quality vs.\ power trade-off: the agent sacrifices power efficiency to maintain link quality above the $-100$~dBm RSRP threshold.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Metric} & \textbf{DQN} & \textbf{PPO} & \textbf{Baseline} \\
\midrule
Mean Reward & 501.28 & 421.62 & $-56.44$ \\
RSRP Violation (\%) & 3.48 & 4.52 & 23.25 \\
Mean Tx Power (dBm) & 46.89 & 47.36 & 40.84 \\
Violation Reduction & 6.7$\times$ & 5.1$\times$ & --- \\
\midrule
...
\begin{enumerate}
    ...
    \item \textbf{Dramatic reduction in RSRP violations.} Both agents achieve $\sim$5$\times$ fewer RSRP violations (3.48--4.52\% vs.\ 23.25\%), demonstrating that RL can effectively manage the dynamic NTN link budget under varying elevation angles, slant ranges, and rain conditions.
...
\begin{enumerate}
    ...
    \item \textbf{RL-Based Power Control:} DQN and PPO agents trained on NTNPowerEnvironment achieve $\sim$5$\times$ fewer RSRP violations (3.48--4.52\% vs.\ 23.25\%) compared to a rule-based baseline, at the cost of $\sim$6~dB additional transmit power.
...
The central finding is that RL agents learn an intelligent signal quality vs.\ power trade-off: by increasing transmit power $\sim$6~dB above the baseline, they reduce RSRP violations from 23.25\% to as low as 3.48\%. This demonstrates the viability of closed-loop RL control over the O-RAN E2 interface for NTN power management under dynamic LEO channel conditions including varying elevation angles, slant ranges, and rain attenuation.
